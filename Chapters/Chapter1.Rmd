...
\hline
  \text{Name} & \text{Function} \\
\hline
  \text{Parameter Assumptions} & x>0,\alpha>0 \\
\hline\
  \text{Probability Density Function} & \frac{\alpha \Big(\frac{x}{\theta}\Big)^\alpha e^-\Big(\frac{x}{\theta}\Big)^\alpha}{x} \\
    \text{f(x)} & \\
\hline
  \text{Cumulative Distribution Function} & 1-\exp\Big(-\Big(\frac{x}{\theta}\Big)^\alpha\Big) \\
    \text{F(x)} & \\
\hline
  \textit{k}^{th}~\text{Raw Moment} & \theta^k \Gamma(1 + \frac{k}{\alpha}) \\
  \mathrm{E}[X^k]  & k>-\alpha \\
\hline
...
theta=100
X=seq(from=0,to=1000,by=1)
plot(x=X,y=dweibull(X,shape=alpha, scale=theta),type="l",ylab="Probability density",col="red",main="Weibull Distribution")
```
...
\begin{array}{l|c}
\hline
  \text{Density Function} & \text{dweibull}(x=, shape=\alpha, scale=\theta) \\
\hline
  \text{Distribution Function} & \text{pweibull}(p=, shape=\alpha,scale=\theta) \\
\hline
  \text{Quantile Function} & \text{qweibull}(q=, shape=\alpha,scale=\theta) \\ 
\hline
  \text{Random Sampling Function} & \text{rweibull}(n=, shape=\alpha,scale=\theta) \\
\hline
\end{array}
\end{matrix}
}
...
\hline
  \text{Parameter Assumptions} & x>0,\alpha>0 \\
\hline\
  \text{Probability Density Function} & \frac{\alpha(\theta/x)^\alpha \exp^{-(\theta/x)^\alpha}}{x} \\
    \text{f(x)} & \\
\hline
  \text{Cumulative Distribution Function} & \exp\Big(-\Big(\frac{\theta}{x}\Big)^\alpha\Big) \\
    \text{F(x)} & \\
\hline
 \textit{k}^{th}~\text{Raw Moment} & \theta^k\Gamma(1-(k/\alpha)) \\
  \mathrm{E}[X^k]  & k<\alpha \\
\hline
...
theta=100
X=seq(from=0,to=1000,by=1)
plot(x=X,y=dinvweibull(X,shape=alpha,scale=theta),type="l",ylab="Probability density",col="red",main="Inv Weibull Distribution")
```
...
\begin{array}{l|c}
\hline
  \text{Density Function} & \text{dinvweibull}(x=, shape=\alpha, scale=\theta) \\
\hline
  \text{Distribution Function} & \text{pinvweibull}(p=, shape=\alpha,scale=\theta) \\
\hline
  \text{Quantile Function} & \text{qinvweibull}(q=, shape=\alpha,scale=\theta) \\ 
\hline
  \text{Random Sampling Function} & \text{rinvweibull}(n=, shape=\alpha,scale=\theta) \\
\hline
\end{array}
\end{matrix}
}
